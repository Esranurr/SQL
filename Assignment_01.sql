...
SELECT order_id, SUM(quantity)AS num_of_product
...
SELECT order_id, SUM(quantity*list_price*(1-discount)) total_amount -- her bir urune odenen miktar
FROM sale.order_item
GROUP BY order_id
...
SELECT order_id, AVG(list_price) avg_product_price
FROM sale.order_item
GROUP BY order_id
...
-- 8. Write a query to pull the first 10 rows and all columns from the product table that have a list_price greater than or equal to 3000.

SELECT TOP 10 *
FROM product.product
WHERE list_price >= 3000
ORDER BY list_price DESC -- optimal


-- 9. Write a query to pull the first 5 rows and all columns from the product table that have a list_price less than 3000.
...
-- 10. Find all customer last names that start with 'B' and end with 's'.
...
-- 11. Use the customer table to find all information regarding customers whose address is Allen or Buffalo or Boston or Berkeley.
